...
/**
 * Each {@link ActivatonMode} should behave differently when it comes to playing samples or loops.
 * Since the might be multiple locations, where samples can be triggered (mouse, keyboard), this behavior
 * is encapsulated in this class. Because the behavior is the same for any usecase (since it only depends on the ActivationMode),
 * you can get a singleton instance by calling getInstance()
